...
点上面的按钮可以直接从 GitHub 部署 `worker/`。Cloudflare 会把 Worker app
clone 到你的 GitHub 账号，自动创建需要的 D1 database，运行 build/deploy 命令，
并把资源绑定到 Worker。
...
### Cloudflare Dashboard 连接 GitHub

也可以完全从 Cloudflare Dashboard 部署，不需要本地 Wrangler：

1. 打开 Cloudflare Dashboard -> Workers & Pages。
2. 选择 Create application。
3. 选择 Import a repository。
4. 连接 GitHub，选择 `sakisakisa-design/superglm` 或你自己的 fork。
